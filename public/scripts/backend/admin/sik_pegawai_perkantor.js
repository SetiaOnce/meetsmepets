...
const _loadDataKantor = () => {
    table = $('#dt-pegawaiPerKantor').DataTable({
        searchDelay: 300,
        processing: true,
        serverSide: false,
        ajax: {
            url: base_url+ 'app_admin/sik/ajax/load_pegawai_perkantor',
        ...
        fnDrawCallback: function (settings, display) {
            ...
            //Custom Table
            $("#dt-pegawaiPerKantor_length select").selectpicker(),
    ...
    $("#dt-pegawaiPerKantor").css("width", "100%");
}
...
const _sinkronisasi = () => {
    ...
    Swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: base_url+ 'app_admin/sik/ajax/sincronize_sik_pegawai_perkantor',
                headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
                type: 'POST',
                dataType: 'JSON',
