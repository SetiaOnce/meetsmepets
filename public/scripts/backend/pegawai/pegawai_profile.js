...
const _loadUserOnProfile = () => {
    $('#changePassUserInfo').hide(), $('#dtlUserInfo').show();
    ...
    $.ajax({
        url: base_url+ "app_pegawai/ajax/pegawai/show",
        type: "GET",
        dataType: "JSON",
        success: function (data) {
            blockUi.release(), blockUi.destroy();
            dataUserSes = data.row;
            let urlThumbUser = data.row.foto;
            let statusUser = `<a href="javascript:void(0);" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-dismiss-="click" title="Status Pegawai Aktif!">
            ...
            if(data.row.status == 0) {
                statusUser = `<a href="javascript:void(0);" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-dismiss-="click" title="Status Pegawai Tidak Aktif!">
                    <!--begin::Svg Icon | path: icons/duotune/general/gen026.svg-->
                    <span class="svg-icon svg-icon-1 svg-icon-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24">
            ...
            let contentUserInfo = `<div class="d-flex flex-wrap flex-sm-nowrap mb-3">
                <!--begin: Pic-->
                <div class="me-7 mb-4">
                    <a href="` +urlThumbUser+ `" class="image-popup" title="` +data.row.nama_pegawai+ `">
                        <div class="symbol symbol-100px symbol-lg-160px symbol-fixed position-relative">
                    ...
                    <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <div class="d-flex align-items-center mb-2">
                                <a href="javascript:void(0);" class="text-gray-900 text-hover-primary fs-2 fw-bold me-1">` +data.row.nama_pegawai+ `</a>
                                ` +statusUser+ `
                            </div>
                            <!--end::Name-->
                        </div>
                    ...
                    <div class="notice d-flex bg-light-primary rounded border-primary border border-dashed  p-6">
                        ...
                        <div class="d-flex flex-stack flex-grow-1 flex-wrap flex-md-nowrap">
                            ...
                            <div class="mb-3 mb-md-0 fw-semibold">
                                ...
                                <div class="fs-6 text-gray-700 pe-7">Lakukan perubahan password secara berkala untuk mencegah orang lain mengetahui password akun anda.</div>
                            </div>
                            <!--end::Content-->
                            <!--begin::Action-->
...
//Change pegawai Password
const _changePass = () => {
    openMethod = 'changePass-userProfile';
    $('#dtlUserInfo').hide(), $('#changePassUserInfo').show();

}
// Handle Reset Password pegawai
$('#btn-resetPassUser').on('click', function (e) {
    e.preventDefault();
    $('#form-changePass')[0].reset();
});
//Handle Enter Submit Form Password pegawai
$("#form-changePass input").keyup(function(event) {
    if (event.keyCode == 13 || event.key === 'Enter') {
        $("#btn-savePassUser").click();
    }
});
// Handle submit Form Password pegawai
$('#btn-savePassUser').on('click', function (e) {
    ...
    if (pass_lama.val() == '') {
        ...
        $('#btn-savePassUser').html('<i class="las la-save fs-1 me-3"></i>Simpan').attr('disabled', false);
        return false;
    }if (pass_baru.val().length < 8) {
        toastr.error('Password baru tidak boleh kurang dari 8 karakter...', 'Uuppss!', {"progressBar": true, "timeOut": 1500});
        pass_baru.focus();
        $('#btn-savePassUser').html('<i class="las la-save fs-1 me-3"></i>Simpan').attr('disabled', false);
        return false;
    }if (repass_baru.val() == '') {
    ...
    Swal.fire({
        ...
        if (result.value) {
            let target = document.querySelector('#cardUserInfo'), blockUi = new KTBlockUI(target, {message: messageBlockUi});
            blockUi.block(), blockUi.destroy();
            let formData = new FormData($('#form-changePass')[0]), ajax_url= base_url+ "app_pegawai/ajax/pegawai/reset_password";
            $.ajax({
                url: ajax_url,
                ...
                success: function (data) {
                    ...
                    blockUi.release(), blockUi.destroy();
                    if (data.status==true){
                        Swal.fire({title: "Success!", html: "Password akun: <strong>" +dataUserSes.nama_pegawai+ "</strong> berhasil diperbarui, Silahkan login ulang ke sistem menggunakan password baru", icon: "success", allowOutsideClick: false}).then(function (result) {
                            let linkUrl = base_url+ 'logout';
...
jQuery(document).ready(function() {
    ...
    new KTPasswordMeter(passwordMeterElement, options);
});