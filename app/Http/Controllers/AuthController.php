...
class AuthController extends Controller
{
    ...
    public function first_login(Request $request) {
        try {
            ...
            $user = UserSystem::where('email', $username)->first();
            if($user) {
                $output = array(
                ...
                $user = UserSystem::where('username', $username)->first();
                if($user) {
                    $output = array(
                        'username' => $user->username,
                        'name' => $user->name,
                    ...
                    return Shortcut::jsonResponse(false, 'Sistem tidak dapat menemukan akun user', 200);
                }
            }
            if($user->is_active=='N') {
    ...
    public function second_login(Request $request) {
        try {
            ...
            $user = UserSystem::where('email', $email)->first();
            $hashedPassword = $user->password;
            if (!Hash::check($password, $hashedPassword)) {
                Shortcut::addToLog('Second step login failed, System cannot find user according to the Password entered !');
                return Shortcut::jsonResponse(false, 'Password yang dimasukkan tidak sesuai, coba lagi dengan password yang benar!', 200, ['error_code' => 'PASSWORD_NOT_VALID']);
            }
            ...
            UserSystem::where('id', auth()->user()->id)->update([
                'is_login' => 'Y',
            ...
            Cookie::queue('username', Auth::guard('web')->user()->username, $expCookie);
            Cookie::queue('email', Auth::guard('web')->user()->email, $expCookie);
